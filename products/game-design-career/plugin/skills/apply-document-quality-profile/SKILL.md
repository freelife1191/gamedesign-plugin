...
2. Before selection, read only `../../references/shared/document-quality/indexes/career.json` and the fixed packaged `../../references/document-quality/template-profile-map.json`. Upper apply rejects a caller-authored map. Load the packaged map only through a plugin root whose root and every ancestor segment, including the first filesystem segment, are non-symlinks; verify its exact raw-byte digest before parsing, then its closed schema, `game-design-career` product identity, template/profile namespace membership, version, and semantic digest. Never preload profile bodies. Normalize IDs and inputs to Unicode NFC, lowercase, and kebab tokens.
...
5. After selection, pass only known `overlayIds` (`mobile`, `live-service`, `pc-console`) and at most one known neutral `presetId`; production upper apply loads those exact bodies from the packaged `pluginRoot` through exact all-ancestor non-symlink paths. Bind the canonical product template map, selected primary, every overlay, and the neutral preset to compiled raw-byte, semantic, identity, and version anchors. Reject a known ID with substituted bytes or data. The upper apply API rejects raw template-map, overlay, profile-preset, and reference-preset objects. Never bulk-load profile bodies or authoring-only evidence.
6. Keep the public low-level profile composer compatible with raw profile-shaped additive overlays/presets and its conflict report. An explicitly injectable test profile/source loader and `templateMapSource` must use the closed `{ "sourceText": "<canonical JSON bytes decoded as UTF-8>" }` shape; upper apply verifies raw-byte digests before parsing and then the same semantic/version/product anchors as packaged loading. At the upper apply boundary, validate the loaded neutral preset separately, reject identity/body mismatch, symlink or path escape, and fail closed when the low-level composition reports any scalar conflict.
