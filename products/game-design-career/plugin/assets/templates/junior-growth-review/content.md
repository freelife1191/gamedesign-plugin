...
## 분기별 근거 {#quarterly-evidence}

각 요구 사항 ID(`requirement-id`)에 프로젝트 이벤트 근거(`project-event-evidence`), 개인·팀 기여 표기, 결정, 결과 상태, 한계, 증빙 산출물(`proof-artifact`)을 연결합니다.

## 성장 약속 {#growth-commitments}

각 목표에는 담당자, 주기, 검토자, 입력 산출물, next-review-date, 목표 깊이 또는 폭, 재평가 규칙을 기록합니다.
...
| `project-event-evidence` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
...
| `next-review-date` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
