...
## 의사결정 흐름 {#decision-chain}

아래 의사결정 항목의 순서를 그대로 사용합니다: `target-competency` → `problem-user` → `evidence` → `hypothesis-intent` → `rules/UI/data/content` → `constraints-alternatives` → `implementation-test` → `result-decision` → `retrospective`.

## 공개 범위 {#publication-boundary}

개인·팀 기여 표기, 제3자 출처, 이용 목적, 권리, 개인정보, 구현 상태, 근거의 한계를 기록합니다.
...
| `problem-user` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
| `evidence` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
| `hypothesis-intent` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
| `rules-ui-data-content` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
| `constraints-alternatives` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
| `implementation-test` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
| `result-decision` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
| `retrospective` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
