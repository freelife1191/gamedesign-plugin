...
version: 1
---
# 경력 단계 목표 {#career-stage-goal}

## 경력 단계와 목표 직무 {#stage-and-target-role}

`stage`: 경력 단계를 기록합니다. 값은 `entry`, `new-hire`, `junior-growth`, `transition` 중 하나를 사용합니다. `target-role`: 근거가 있을 때만 직무군과 레벨을 적습니다.

## 목표 정의 {#goal-contract}

달성할 결과, 성공 근거, 기한, 담당자, 검토일을 분명히 정합니다. 정답인 경력 경로가 확정되지 않았다면 여러 경로를 함께 남깁니다.

## 작업 기록 {#working-record}

| 항목 ID | 현재 상태 | 근거 또는 다음 작업 | 담당자 |
| --- | --- | --- | --- |
| `stage` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
| `target-role` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
| `goal` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
| `success-evidence` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
| `owner` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
| `approval-status` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
| `review-date` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 함께 기록합니다. | artifact-owner |
