...
For every record, retain `sourceUrl`, `postedDate`, `retrievalDate`, and `sourceType`. If a posted date is unavailable, record `null` and the resulting freshness limitation rather than guessing.
...
A posting-specific requirement remains attached to its source. Label a repeated market signal only when the same normalized requirement appears in at least two distinct primary postings within the declared scope. Report its count, denominator, source IDs, geography, and date range. “Repeated” means repeated in the sample; it does not prove prevalence outside the sample.
