...
version: 1
---
# 프로덕션 범위 및 위험 {#production-scope-risk}

## 범위·위험 관리 기준 {#scope-and-risk-contract}

각 scope-id에 핵심 루프 기여도, MoSCoW 분류, 소요 공수, 의존성, 유지보수 부담, 라이선스 또는 외주 위험, 프로토타입 가설, 마일스톤, 담당자, 완료 정의, 중단 기준을 기록한다.

## 투입 확정 게이트 {#commitment-gate}

목표 경험 근거, 프로토타입 결과, 가용 인력 근거, 명시된 담당자, 측정 가능한 완료 정의, 중단 기준, 비상 계획, 재개 조건이 없으면 큰 규모의 투입을 승인하지 않는다.

## 작성 기록 {#working-record}

| 항목 ID | 상태 | 근거 또는 다음 작업 | 담당자 |
| --- | --- | --- | --- |
| `scope-id` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `core-loop-contribution` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `moscow` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `effort` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `dependency` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `maintenance` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `rights-outsource-risk` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `prototype-hypothesis` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `definition-of-done` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `kill-criterion` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
