...
version: 1
---
# 핵심·동기 부여 루프 {#core-motivation-loop}

## 루프 명세 {#loop-contract}

각 loop-step에서 발동 조건, 플레이어 입력, 시스템 반응, 피드백, 보상, 동기 욕구, 유의미한 선택, 반복, 실패·복구, 측정 가능한 결과를 추적한다.

## 과몰입 방지 {#compulsion-safety}

유의미한 선택을 가려진 확률, 강요하는 긴급성, 손실 프레이밍, 끝없는 단계 상승으로 대체하지 않는다. 중단 조건과 플레이어 보호 지표를 기록한다.

## 작성 기록 {#working-record}

| 항목 ID | 상태 | 근거 또는 다음 작업 | 담당자 |
| --- | --- | --- | --- |
| `loop-step` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `player-input` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `system-response` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
...
| `motivation-need` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `meaningful-choice` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `failure-recovery` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `metric` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
