...
2. Before selection, read only `../../references/shared/document-quality/indexes/studio.json` and the fixed packaged `../../references/document-quality/template-profile-map.json`. Upper apply rejects a caller-authored map. Load the packaged map only through a plugin root whose root and every ancestor segment, including the first filesystem segment, are non-symlinks; verify its exact raw-byte digest before parsing, then its closed schema, `game-design-studio` product identity, template/profile namespace membership, version, and semantic digest. Never preload profile bodies. Normalize IDs and inputs to Unicode NFC, lowercase, and kebab tokens.
