...
Require one validated quality-profile selection record per compatible artifact and apply it before content generation and asset planning. Advance only `draft -> structurally-complete -> evidence-reviewed -> visual-reviewed -> document-approved`; do not skip states. Carry the transition in the immutable state envelope, and require a digest-bound external artifact-inspection receipt covering every required ID before `structurally-complete`; requirements or caller state strings cannot complete it.
