...
description: Use when a game-design request is broad or ambiguous, names a Studio case ID, or no skill obviously fits.
---
...
`선택 스킬`에는 이 대표 진입점의 ID인 `game-design-studio`를 적지 않습니다. `route-receipt.json`의 `routeId`가 실제로 넘긴 전문 스킬이나 오케스트레이터 ID를 기록합니다.
