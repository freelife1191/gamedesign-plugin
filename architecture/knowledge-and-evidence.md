...
`shared/knowledge/trends/`는 변하는 주장을 관리합니다. 현재 register는 2026년 8월 11일까지 확인한 1차 출처 16개를 기록합니다.
...
`analyze-game-design-references`는 “이 게임을 분석해 줘”라는 한 번의 요약 대신 다음 순서로 증거를 축적합니다.

1. 분석 brief에서 대상 게임, 장르, 플랫폼, 비교 질문, 지역·버전과 제외 범위를 닫습니다.
2. 공식 사이트·업데이트 노트·게임 화면·위키·공략 자료의 locator, 날짜, 권리와 한계를 증거 등록부에 남깁니다.
3. 전투·조작·성장·수집·경제·보상·콘텐츠·소셜·비즈니스 모델(BM)·재방문(리텐션)·라이브 운영(LiveOps)·사용자 경험(UX)을 고정 체크리스트가 아니라 관찰된 시스템 인벤토리로 만듭니다.
4. 각 시스템을 입력(Input)·처리(Process)·결과(Output)·연결 시스템(Connected System)으로 연결하고 핵심 반복(Core Loop), 장기 반복(Meta Loop), 진행·경제·콘텐츠·재방문 구조를 분석합니다.
5. 확인된 규칙, 추정 규칙, 반례·충돌과 추가 확인 질문을 분리합니다. 근거가 없는 항목은 `미정`이며 존재한다고 간주하지 않습니다.
6. 현재 프로젝트에 대한 `adopt`, `adapt`, `reject`, `hold` 제안을 만들되, 원작의 성공 원인이나 전환 제안을 사실·승인으로 승격하지 않습니다.

분석 작업 공간(workspace)은 브리프(brief), 증거 등록부(evidence register), 시스템 지도(system map), 주장·질문 집합과 설계 전환 제안을 다시 읽을 수 있게 보존합니다. 서로 충돌하거나 반례인 증거는 긍정 주장의 근거로 계산하지 않습니다.

## 프로젝트 기억과 용어 사전

프로젝트 기억은 분석·플레이테스트·검토에서 다시 쓸 교훈을 후보로 남길 수 있지만, 승인 전 후보는 검색 결과에 적용되지 않습니다. 승인된 기억도 원래 출처의 SHA-256, 적용 범위, 검토·만료 시점이 현재 작업과 맞아야 합니다. 로컬 `.game-design/` 저장소가 없거나 손상되면 주 작업은 기억 없이 계속합니다.

용어 사전은 한국어·영어 선호 용어, 정의, 영어 지역 규칙(locale)과 문서별 영향을 별도 산출물(artifact)로 관리합니다. 후보 추출과 충돌 진단은 읽기 전용으로 수행할 수 있지만, 의미가 달라질 수 있는 본문 치환과 용어집 상태 전이는 이름이 기록된 사람의 정확한 결정과 스냅샷 영수증을 요구합니다. 승인 후에도 먼저 수정안과 변경 비교(diff)를 만들고 문서 반영은 별도 사람 검토를 거칩니다.

컷씬 이미지에도 같은 출처 추적(provenance) 원칙을 적용합니다. 기준(master) 이미지 바이트(bytes), 프롬프트 계보(prompt lineage), 비용 견적과 생성 승인 기록(receipt)을 묶고, 각 단계(wave)는 승인된 참조 다이제스트(reference digest)를 생성 제공자(provider) 호출 전에 다시 확인합니다. 한글 픽셀 텍스트가 없는 이미지는 호스트 `image_gen`을 먼저 사용하고, 한글 문자가 반드시 들어가야 할 때만 explicit OpenAI `gpt-image-2` 경로를 요구합니다. 유료 품질은 `low`가 기본이며 `medium`은 선택한 마스터·키 이미지, `high`는 비용을 별도 승인한 영상용 핵심 프레임·게임 원화의 예외로 제한합니다. 생성 결과는 연속성(continuity) 검토와 사람 승인을 거치기 전 제작 후보(production candidate)가 아닙니다.
