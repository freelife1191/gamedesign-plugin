...
> 🧭 아이디어 한 줄을 검토 가능한 게임 기획 문서로 발전시키고, 그 결과를
> 학습·포트폴리오 작업까지 이어 주는 게임 기획 도구 모음입니다.

### 🧩 어떤 플러그인인가요?

이 저장소에는 목적이 다른 두 플러그인이 들어 있습니다.

- **Game Design Studio**는 게임의 방향, 규칙, 콘텐츠, 화면 흐름, 경제와
  제작 범위를 설계하고 검토합니다.
- **Game Design Career**는 직무 탐색, 역기획, 학습 계획, 포트폴리오,
  면접과 주니어 성장 계획을 정리합니다.
- 두 제품을 함께 쓰면 Studio에서 검토한 기획을 Career의 포트폴리오와
  면접 준비 자료로 연결할 수 있습니다. 두 결과 폴더와 승인 기록은
  서로 섞지 않습니다.

간단한 요청은 플러그인이 목적을 파악해 필요한 스킬과 검토 역할을 고릅니다.
정교한 통제가 필요할 때만 상세 가이드의 스킬 ID와 작업 순서를 직접 지정하면
됩니다. 결과를 자동으로 승인하지 않으며, 이름 있는 사람이 사실·권리·범위를
검토한 뒤 승인하거나 보류합니다.

### 🎯 왜 만들었나요?

게임 기획 자료는 좋은 설명이 많아도 실제 프로젝트에서 바로 쓰기 어렵습니다.
아이디어, 경험담, 실무 원칙과 최신 정보가 한데 섞이면 무엇을 근거로 삼아야
하는지도 불분명해집니다.

이 플러그인은 흩어진 자료를 그대로 답처럼 복사하지 않습니다. 질문을 작업
순서로 바꾸고, 모르는 내용은 `미정`으로 남기며, 근거와 선택 이유를 결과물에
함께 기록하도록 만들었습니다. 초안 작성에서 검토, 수정, 시각화, 내보내기까지
같은 기준을 이어 가는 것이 개발 취지입니다.

### 👥 이런 분께 잘 맞습니다

| 사용자 | 도움받을 수 있는 일 |
| --- | --- |
| 🎓 게임 기획 학생·입문자 | 막연한 아이디어를 규칙, 화면 흐름과 검증 질문으로 구체화 |
| 💼 취업 준비생·직무 전환자 | 목표 직무, 역량 차이, 학습 계획, 포트폴리오와 면접 준비를 연결 |
| 🎮 현업 기획자·인디 개발자 | 시스템·콘텐츠·경제·제작 범위를 검토하고 빠진 결정과 위험을 확인 |
| 🧑‍🏫 팀 리드·멘토·교육자 | 기획 근거, 검토 의견, 수정 이력과 사람 승인 경계를 일관되게 관리 |

### 🛠️ 이 플러그인으로 할 수 있는 일

- 게임의 방향과 핵심 재미를 짧은 기획 요약서로 정리합니다.
- 규칙, 상태, 예외, 데이터, 콘텐츠, UX, 경제와 제작 위험을 검토합니다.
- 직무 탐색, 역기획, 학습 계획, 포트폴리오, 면접 답변을 근거와 연결합니다.
- 긴 문서의 어색한 한국어 표현을 다듬되 사실, 수치, ID와 승인 상태는
  바꾸지 않습니다.
- Skillstead와 Archify로 흐름도와 시스템 구조도를 만들고 검증합니다.
- 승인된 결과를 MD·PDF·DOCX·PPTX로 내보낼 준비를 합니다.
- 필요하면 마스터 이미지를 기준으로 게임 이미지의 생성 계획과 검토 기록을
  남깁니다.

### 💬 처음에는 이렇게 물어보세요

스킬 이름이나 사례 번호를 몰라도 됩니다. 가진 자료와 원하는 결과를 한 문장으로
말하고, 모르는 항목은 `미정`이라고 적으세요.
...
좋은 첫 질문에는 세 가지만 있으면 됩니다.

1. **무엇을 만들지:** 전투 시스템, 온보딩, 포트폴리오 사례처럼 결과를 적습니다.
2. **무엇을 가지고 있는지:** 메모, 관찰 기록, 기존 문서와 공개 가능한 범위를
   알려 줍니다.
3. **무엇이 아직 미정인지:** 결정하지 못한 내용과 사람이 확인할 항목을 남깁니다.

### 📚 어떤 자료를 참고했나요?

개발의 주요 출발점 가운데 하나는 게임 기획자 유리링의
[YouTube 채널](https://www.youtube.com/@GameDesignerYuriring)과 저장소에 정리한
관련 리포트입니다. 신입 면접, 말하기와 협업, 포트폴리오, 학벌·전공·자격증,
게임 개발 경험, 취업 준비 전략을 다룬 유리링 관련 리포트 6편을 참고했습니다.

저장소의 유리링 관련 리포트 6편은 채널 내용을 학습용으로 정리한 로컬 자료이며,
공식 자막이나 채널 운영자의 검수본은 아닙니다. 영상 속 경험과 조언도 모든 팀과
프로젝트에 그대로 적용되는 보편 법칙으로 다루지 않습니다.
...
자료는 역할이 다른 세 층으로 나눕니다.

| 근거 층 | 저장소에서의 역할 | 답변에 쓰는 방식 |
| --- | --- | --- |
| 원문 (Source) | 원문 49편과 출처·범주를 보존 | 관찰, 경험담과 작성 맥락을 확인 |
| 핵심 원칙 (Core) | 핵심 실무 원칙 7개 문서로 정리 | 여러 작업에 반복해서 쓸 검토 질문과 기준 제공 |
| 현재 자료 (Current) | 현재 1차 자료 16건, 확인일 2026-08-11 | 도구·플랫폼·채용처럼 바뀔 수 있는 사실을 재확인 |

시점에 따라 달라지는 주장은 사용 시점에 최신 공식 자료를 다시 확인합니다.
자료에서 확인되지 않은 수치와 경험은 만들지 않고 `미정`, 가정 또는 다음 검증
항목으로 남깁니다. 이 플러그인은 흥행·매출·채용 합격을 보장하지 않습니다.
최종 결과와 공개 여부는 이름 있는 사람이 근거와 권리를 검토해 결정합니다.

### 🔗 레퍼런스와 더 읽을 문서

- [게임 기획자 유리링 YouTube 채널](https://www.youtube.com/@GameDesignerYuriring)
- [원문 49편과 범주 목록](shared/knowledge/reference-index.json)
- [자료를 원문·핵심 원칙·현재 자료로 나누는 기준](architecture/knowledge-and-evidence.md)
- [현재 1차 자료 16건과 확인일](shared/knowledge/trends/source-register.json)
- [Studio와 Career 전체 사용자 가이드](guides/README.md)
- [사실·추론·가정 구분 기준](architecture/knowledge-and-evidence.md#사실추론가정-경계)

> ⚠️ 저장소의 MIT 라이선스는 원문과 제3자 자료의 권리를 다시 허가하지
> 않습니다. 원문을 공개하거나 재사용하기 전에 해당 자료의 이용 조건과 권리를
> 따로 확인하세요.
...
Studio 생성본 (snapshot)은 에이전트 12개, 설치 스킬 18개, 템플릿 15개와
지원 스크립트 16개를 포함합니다.
...
Career 생성본 (snapshot)은 에이전트 10개, 설치 스킬 18개, 템플릿 15개와
지원 스크립트 16개를 포함합니다.
