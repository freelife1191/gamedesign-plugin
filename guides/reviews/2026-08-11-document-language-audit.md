...
2026-08-11에 `shared/templates`, `shared/knowledge/trends`, 프롬프트 카탈로그 원본, Studio와 Career의 사용자 결과 템플릿을 검사했습니다. 제품별 30개 `content.md`의 보이는 표제·설명·표 작업 지시와 각 템플릿의 90개 content/evidence/export 계약 파일을 함께 확인했습니다. 원천을 고친 뒤 `plugins/` 설치 스냅샷은 표준 빌드로 다시 만들었습니다. README·빠른 시작·대표 샘플은 별도 초보자 용어 계약으로 검증하며, vendor와 이미지·도식 자료는 각각 전용 계약으로 검증합니다.
...
- 초보자 용어: `비전 기둥`은 `게임 방향 원칙`, `게임 기획 브리프`는 `게임 기획 요약서`로 바꾸고, 내부 식별자 `vision-pillars`와 `game-design-brief`는 괄호 안에 그대로 보존했습니다. `player promise`, `anti-pillar`, `non-goal`, `blocker receipt`처럼 설명을 방해하던 계약어도 플레이 경험의 약속, 하지 않을 설계 원칙, 제외 목표, 중단·재개 기록으로 풀어 썼습니다
...
감사기는 번역투의 자동 생성 문장, 근거 없는 과장, 설명 없는 영어 우선 표제, 반복 결론을 고심각도로 표시합니다. 한국어 접두어 뒤에 영어 한 단어만 남기거나, 짧은 영어 지시문을 붙여 검사를 피하는 문장도 차단합니다. UX, UI, LiveOps, API와 stable ID·명령어·파일 경로는 게임 제작 문맥의 표준 용어 또는 보호 대상이므로 표시하지 않습니다. 문장이 자연스럽더라도 초보자에게 낯선 직역 용어는 별도 표시명 계약에서 차단합니다.
