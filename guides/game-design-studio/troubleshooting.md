...
| 설치했지만 스킬이 호출되지 않음 | 설치 이전 채팅 또는 stale session을 계속 사용 | App의 새 채팅인지, CLI의 새 세션인지 확인 | App은 새 채팅, CLI는 새 세션을 시작 | `설치된 Game Design Studio로 이 아이디어를 brief로 정리해줘.` | 기존 채팅과 로컬 Artifact |
| CLI가 플러그인을 못 고름 | marketplace selector 누락 또는 오타 | `codex plugin list`에서 plugin ID와 marketplace 이름 확인 | `game-design-studio@game-design-suite`로 다시 설치하거나 `/plugins`의 해당 marketplace 탭 사용 | `$game-design-studio:orchestrate-game-design-project 기존 Artifact를 이어서 진행해줘.` | 설치된 다른 플러그인과 marketplace 등록 |
...
| API key가 있는데도 호스트 이미지가 선택됨 | 기본 `IMAGE_PROVIDER=codex-first`는 key 존재와 유료 승인을 분리함 | provider decision과 host capability 확인 | 호스트 결과를 먼저 검토하고, 필요할 때만 비용 안내 뒤 `IMAGE_PROVIDER=openai`를 명시해 새 승인을 받음 | `호스트 결과를 보존하고 gpt-image-2 low 비용을 알려준 뒤 승인 전에는 실행하지 마.` | prompt, 호스트 결과, 유료 전환 제안과 승인 상태 |
| 한글 문자가 이미지에 나오지 않음 | 한글 삽입 경로를 선언하지 않았거나 호스트 경로를 사용함 | `IMAGE_EMBEDDED_TEXT_LOCALE`, provider, model 확인 | `ko-KR` + explicit OpenAI + `gpt-image-2`로 새 비용·승인을 만든 뒤 실행 | `이미지 안에 '출격 준비'를 넣어야 해. gpt-image-2 low 비용을 먼저 알려주고 승인 전에는 생성하지 마.` | exact 한글 문구, 비용·승인, 생성 receipt |
