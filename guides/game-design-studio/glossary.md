...
이 가이드는 Studio 문서에서 발견한 게임 기획 용어를 후보로 모으고, 사람이 승인한 용어만 스냅샷으로 기록하는 방법을 설명합니다. 용어 사전은 원문을 자동 치환하지 않으며, 기획 문장의 뜻이나 출처를 대신 판단하지 않습니다.

## 용어 후보를 요청하기

```text
@Game Design Studio 전투 시스템 문서에서 용어 후보를 찾아 줘. TERM-PLAYER-POWER의 한국어 선호 표현은 플레이어 파워, 영어 선호 표현은 Player Power로 제안하고, 원문은 자동 치환하지 마.
```

```text
$game-design-studio:maintain-game-design-glossary 후보 용어의 정의, 적용 범위, 근거 ID, 금지 표현과 혼동할 개념을 제안해 줘. 사람 승인 전에는 proposed 상태로만 남겨 줘.
```
...
8. **impact review**: 폐기·대체가 영향을 주는 문서와 표현을 사람이 검토합니다
...
용어 후보의 관찰 근거와 시스템 맥락은 [경쟁작 분석](reference-analysis.md)에서 확인합니다. 분석 결과가 `hold`이거나 근거가 부족하면 용어도 자동 승인하지 않고 확인 질문을 남깁니다.
