...
장르 관례와 오버레이는 조사 질문을 고르는 단서일 뿐입니다. 프로젝트 규칙이나 설계 전환 제안의 자동 의무가 아닙니다.
...
설계 전환 제안은 모두 `pending-review`로 시작하며 `validationState: not-run`입니다. 사람은 근거, 프로젝트 제약, 위험, 검증 방법을 확인한 뒤에만 채택 여부를 결정합니다.
