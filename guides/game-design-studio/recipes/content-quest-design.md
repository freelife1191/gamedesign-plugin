...
5. 퀘스트 관계는 `visualize-game-design` SVG로만 도식화합니다. illustration이 필요할 때만 `plan-image-assets`를 실행하며 `prompt-only`는 생성하지 않고, `select`/`required`/`all`은 명시된 stable ID 범위만 생성합니다.
