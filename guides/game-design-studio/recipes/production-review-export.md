...
5. production handoff 관계는 `visualize-game-design` SVG로, diagram 아닌 illustration asset은 `plan-image-assets`로 분리합니다. `IMAGE_GEN_MODE=prompt-only`는 계획만, `select`는 사람 receipt 후, `required`/`all`은 declared manifest ID만 처리합니다.
