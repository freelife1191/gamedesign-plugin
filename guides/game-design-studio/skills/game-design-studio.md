...
```text
@Game Design Studio 협동 탐험 게임을 어디서부터 시작할지 모르겠어. 요청을 다섯 항목으로 정리하고 담당 스킬 하나를 골라 라우팅 영수증을 남겨 줘. 모르는 정보는 미정으로 남겨 줘.
```
...
```text
@Game Design Studio 전투·경제·콘텐츠가 한 번에 섞인 요청이야. 소유 제품 하나와 실행 경로 하나로 좁히고 나머지는 후속 결정으로 라우팅 영수증에 적어 줘.
```
...
```text
@Game Design Studio ST-G04로 시작하고 커리어 근거도 있어야 해. 사례 ID를 실행 경로로 바꾸고 소유자와 공급자를 나눠 라우팅 영수증을 만들어 줘. 상대 제품이 없으면 그 사실을 그대로 적어 줘.
```
...
상대 제품(Career) 소유로 판정한 요청은 근거만 요청하는 단방향 인계로 나눕니다. 인계는 한 요청에 한 번이고, 상대 제품이 없거나 비활성이면 증거를 지어내지 않고 blocker로 남깁니다.
