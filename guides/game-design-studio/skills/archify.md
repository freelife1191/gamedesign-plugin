...
`archify`는 게임 기획의 시스템 구조, 작업 흐름, 상태 변화, 데이터 흐름을 읽기 쉬운 한국어 HTML 도식으로 만듭니다. 검증을 마친 JSON 원본, 대화형 HTML, 검증 기록을 함께 남깁니다.
...
```text
$game-design-studio:archify 전투 보상 시스템의 구성 요소와 검토 흐름을 한국어 아키텍처 HTML로 정리하고 검증 기록도 남겨 줘.
```
...
`assets/` 아래에 도식 JSON과 HTML, 검증 기록을 남깁니다. 예상 결과는 구조를 설명하기 위한 자료이며, 담당자의 승인이나 구현 완료를 뜻하지 않습니다.
