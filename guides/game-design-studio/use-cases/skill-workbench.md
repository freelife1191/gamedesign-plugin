...
| 스킬 | 직접 호출 신호 | 피할 때 | 입력 | 결과 | 다음 스킬 | 상세 가이드 |
| --- | --- | --- | --- | --- | --- | --- |
| `upgrade-game-design-suite` | `$game-design-studio:upgrade-game-design-suite` — 설치된 버전과 공개된 릴리스 비교 | 설계·문서 작업을 진행할 때 | 설치 위치·marketplace 종류·사람의 선택 | 버전 비교·검증한 재설치 계획·선택 기록 | 선택한 처리 뒤 새 세션 | [설치와 업데이트](../installation.md) |
