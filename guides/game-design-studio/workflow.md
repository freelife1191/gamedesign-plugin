...
각 작업은 현재 프로젝트에서 승인된 Studio·공통 기억을 먼저 조회하되 출처가 달라졌거나 검토·만료 시점을 지난 기록은 제외합니다. 결과를 검증한 뒤에는 재사용 가치가 있는 교훈만 후보로 남기며 자동 승인하지 않습니다. 기억을 끄거나 조회·저장에 문제가 생기면 아래 워크플로는 기억 없이 그대로 진행됩니다. 설정과 관리 방법은 [Studio 프로젝트 기억](memory.md)을 확인하세요.
