...
## 이 요청으로 받는 결과
입력은 “두 명이 바람을 타고 섬을 복구한다”는 한 줄 설명뿐인 허구 데이터다. 가정 및 제안: 두 명 협동과 15분 세션은 확인 전 범위 후보일 뿐이다. 가정: 항해 조작이 재미를 만든다는 것은 아직 추론이다. 제안: 첫 시제품은 돛 조작과 복구 선택 한 쌍만 검증한다. 다음 질문: 실패한 항해 뒤 두 플레이어가 다시 합류할 방법은 무엇인가?
...
## 보호한 가정
가상의 사례이며 허구 데이터만 사용합니다. 확인되지 않은 내용은 사실처럼 채우지 않고 미정으로 남깁니다.
...
결과 보장 없음: 스킬 실행과 reviewer finding은 승인을 대신하지 않습니다.
