...
- 버전: `Google Chrome 151.0.7922.170`
...
| `app-cli-install-flow` | 상단 `설치 전 확인` 구역 뒤에 App·CLI 설치 경로가 이어지고 셋이 한 화면에 들어옴 | 구역 제목·카드·명령과 두 사전 확인 카드 본문의 잘림·넘침 없음 | 설치 전 확인에서 App 경로로 내려간 뒤 각 설치 경로는 왼쪽에서 오른쪽으로 진행; 화살표와 카드 사이 간격 정상 | 두 설치 가이드의 UTF-8 BOM 진단, Git·로컬 마켓플레이스 구분, Windows·Ubuntu 설치·선택 제거·재설치·전체 정리와 잔재 확인을 반영 |
| `canonical-artifact-lifecycle` | 작성→검토→담당자 결정과 하단 결과가 한 화면에 보임 | 담당자 확인 표시, 상태 문구와 하단 영역이 경계 안에 정상 배치됨 | 왼쪽에서 오른쪽으로 진행한 뒤 아래의 자산·내보내기 준비로 연결되며 화살표 간격 정상 | Studio·Career 작업 흐름의 기준 결과 폴더·전문 검토·담당자 결정을 반영 |
| `image-generation-mode-routing` | `생성 요청?`에서 기본값과 생성 요청 경로가 즉시 구분됨 | 모든 설정값과 한글 문구가 카드·하단 공통 경계 안에 정상 배치됨 | 생성 요청은 `select`·`required`·`all`로 동등하게 나뉘고 네 결과가 하단 공통 경계에서 합쳐짐 | 두 이미지 가이드의 `prompt-only`, `select`, `required`, `all`, 선택 기록과 자산 목록 경계를 반영 |
...
| `image-asset-lifecycle` | 콘셉트 초안→시각 검토→문서 승인 완료→제작 검토 후보가 명확함 | 검토자 표시·상태 이름·하단 영역이 모두 경계 안에 정상 배치됨 | 왼쪽에서 오른쪽으로 진행한 뒤 권리 검토 단계로 내려가며 화살표와 카드 사이 간격 정상 | 두 이미지 가이드의 `concept-draft`, `document-approved`, `production-candidate` 상태와 지정된 담당자 검토를 반영 |
| `document-export-flow` | 사전 확인, 공통 상태, MD·PDF·DOCX·PPTX별 검사가 즉시 구분됨 | 한글 상태 이름과 기술 상태 ID, 형식 이름이 모두 경계 안에 정상 배치됨 | 공통 상태에서 형식별 카드 네 개로 나뉘며 화살표와 카드 사이 간격 정상 | 두 내보내기 가이드의 `not-requested`·`blocked`·`pending`·`unavailable` 상태와 형식별 화면 검사를 반영 |
...
| `suite-entry-routing-flow` | 요청 도착→대표 스킬→선택 근거와 실행의 세 구역이 위에서 아래로 바로 읽힘 | `route-receipt.json`, `routeId`, ST-C01 글리프와 3열 카드가 경계 안에 정상 배치됨 | 구역 사이 아래 방향 화살표 두 개(`y=322`, `y=562`)와 구역 안 왼쪽→오른쪽 화살표; 카드 경계 앞 12px 간격 | 두 대표 스킬 가이드의 요청 정리, 담당 제품 선택, 여섯 줄의 경로 선택 기록, 전문 스킬·오케스트레이터와 최대 세 개의 검토 역할, 한 번만 전달하는 인계 원칙을 반영 |
| `suite-handoff-ownership-flow` | 담당 제품 확정→근거 제공→멈춤과 승인이 한 화면에서 구분됨 | `returnToSkill`, `requestedOutputs`, `sourceArtifactIds` 식별자와 한글 설명이 카드 경계 안에 정상 배치됨 | 상단은 왼쪽에서 오른쪽으로 진행하고 두 번째 구역에서 하단으로 나뉘며, 근거는 담당 제품 쪽으로 한 번만 돌아감 | 제품 간 인계 계약의 최종 담당 제품, 근거 제공 제품, 요청·반환 키, 인계 1회 제한, 상대 부재 시 차단 요인과 담당자 승인 경계를 반영 |
...
초기 검사에서 `plugin-selection-flow`의 `공통 marketplace 설치` 문구가 넘칠 가능성을 발견해 `marketplace 설치`로 줄였고, 다시 검사해 오류와 경고가 없음을 확인했습니다. 이후 전체 SVG와 PNG를 다시 만들고 화면 크기를 달리해 두 차례 검토했습니다. 2026-08-16에는 새 이미지 생성 경로와 프로젝트 기억 재사용 흐름을 원본 해상도로 확인하고, 붐비던 분기 이름과 화살표 끝점을 정리했습니다. 2026-08-19에는 대표 진입, 단방향 인계, 업데이트 승인 도식을 추가하고 설치 도식에 UTF-8 사전 확인과 마켓플레이스 유형을 넣었습니다. 네 SVG는 오류·경고 0건을 통과했고, 네 PNG는 모두 `2800×1800`과 정상 종료를 확인했습니다. 2026-08-20에는 공식 최신 태그가 `svg-infographic/v0.10.0`임을 다시 확인하고 같은 고정본으로 설치 수명주기와 대표 오케스트레이션 도식을 보정했습니다. 두 SVG는 오류·경고 0건을 통과했으며 Chrome 151.0.7922.170에서 `2800×1800` PNG로 렌더링한 뒤 한글 깨짐, 넘침, 카드 경계와 화살표를 다시 확인했습니다.

## Archify 전체 시스템 구조 재검사 — 2026-08-20

- `suite-plugin-system-architecture`는 설치·업데이트·전체 정리, 대표 스킬의 경로 선택, 기준 결과물과 자동 검증, 담당자 검토·승인과 재개의 네 안내 보기를 제공합니다. 대표 스킬은 여섯 줄의 경로 선택 기록을 남기고 전문 스킬 또는 오케스트레이터와 최대 세 개의 검토 역할을 고릅니다.
