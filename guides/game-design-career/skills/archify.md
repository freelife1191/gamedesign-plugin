...
경력 Artifact의 `content.md`와 근거를 읽고 도식 유형을 고릅니다. JSON 원본을 검증한 뒤 HTML과 영수증을 남기며, 역할 적합성 판단은 멘토가 합니다.

## 생성 파일과 결과 구조

`assets/`에 도식 JSON·HTML·검증 영수증을 남깁니다. 예상 결과는 학습 대화용 경력 흐름 도식이며 채용 승인 증거가 아닙니다.
