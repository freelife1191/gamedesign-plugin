...
4. marketplace `game-design-suite`에서 **Game Design Career**를 골라 설치합니다.
...
`codex plugin marketplace upgrade`는 구성된 **Git marketplace**를 refresh하는 명령입니다. 설치된 Career 자체를 새 snapshot으로 교체하지 않으므로, 필요한 버전을 확인한 뒤 명시적으로 다시 설치합니다.
