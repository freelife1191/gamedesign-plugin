...
### 이 요청으로 받는 결과
계절 시장 아이디어는 경제 설계 경로 하나로 보내고, 산출물은 경제 균형 문서로 제한했습니다. 완료 관문과 다음 책임자는 프로젝트 담당자가 확인해야 합니다. (ID: studio:orchestrate-game-design-project:beginner; 파일: game-design/studio-production/orchestrate-beginner/content.md)
...
### 이 요청으로 받는 결과
비행 규칙 명세에서 사용자 경험 흐름과 콘텐츠 퀘스트 문서로 이어지는 순서를 지정했습니다. 역할별 검토 사항은 세 건으로 제한했으며 관문 통과 여부는 결정 담당자가 정합니다. (ID: studio:orchestrate-game-design-project:standard; 파일: game-design/studio-production/orchestrate-standard/content.md)
...
### 이 요청으로 받는 결과
해저 정원 협동 흐름은 접근성 관문에서 멈춘 상태로 남기고, 기획·사용자 경험 검토 의견을 하나의 결정 대기열에 모았습니다. 접근성 책임자의 답변이 들어온 뒤 해당 관문부터 다시 시작합니다. (ID: studio:orchestrate-game-design-project:advanced; 파일: game-design/studio-production/orchestrate-advanced/content.md)
