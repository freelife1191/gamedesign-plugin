...
### 이 요청으로 받는 결과
표지에는 ASSET-COVER-01 자리 하나를 두고 ‘규칙표와 테스트 메모가 놓인 작업 책상’이라는 대체 문구를 붙였습니다. 이미지는 생성하지 않았으며 출처와 사용 권리는 미정으로 남겼습니다. (ID: career:plan-image-assets:beginner; 파일: career/visual/plan-image-assets/beginner/content.md)
...
### 이 요청으로 받는 결과
사례 2에는 수정 전후 화면 두 장을 배치하고 각 이미지의 출처, 권리 담당자, 대체 문구를 기록하도록 정했습니다. 실제 파일 생성과 공개 사용은 권리 담당자 검토 뒤 별도 단계에서 진행합니다. (ID: career:plan-image-assets:standard; 파일: career/visual/plan-image-assets/standard/content.md)
...
### 이 요청으로 받는 결과
프로필, 대표 사례, 과정 설명에 쓸 이미지 자리를 각각 고유 자산 식별자로 나눴습니다. 발표 자료 담당자에게 넘길 크기와 대체 문구는 적었지만 공개 가능한 자산인지는 아직 검토 중입니다. (ID: career:plan-image-assets:advanced; 파일: career/visual/plan-image-assets/advanced/content.md)
