...
| manifest | 어떤 파일·형식·자산·검사를 준비했는지 stable ID로 적는 작업 목록입니다. |
| stable asset ID | 계획·생성·검토를 거쳐도 바뀌지 않는 이미지 자산의 고유 이름입니다. |
| receipt | host가 실제 사용자 선택·승인과 증거를 변경 불가능한 구조로 기록한 영수 기록입니다. |
| slot | 템플릿에서 이미지나 도식처럼 필요한 자산을 넣을 자리와 조건을 가리키는 항목입니다. |
| host | 지금 대화나 세션을 실행하며 파일 생성 capability를 제공할 수도 있는 Codex 실행 환경입니다. |
