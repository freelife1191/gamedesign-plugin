...
const mutations = {
  "evidence-tier": {
    ...
    actual: true,
    scenario(moduleUrl) {
      return `const api = await import(${JSON.stringify(moduleUrl)}); const records = api.registerReferenceEvidence({ records: [{ evidenceId: "ev-lead", referenceId: "ref-alpha", contextId: "ctx-alpha", systemIds: ["retention"], sourceType: "community", claimKind: "observation", claim: "Discovery lead.", availability: "available", limitation: null, verificationQuestion: null }] }); const evidenceById = new Map(records.map((item) => [item.evidenceId, item])); const result = api.validateClaimAgainstEvidence({ claim: { claimId: "claim-retention", systemIds: ["retention"], evidenceIds: ["ev-lead"], kind: "observation", category: "retention", causal: true }, evidenceById }); mutationEqual(result.ok, false);`;
    },
  },
  "fact-inference": {
    target: "shared/scripts/lib/reference-evidence.mjs",
