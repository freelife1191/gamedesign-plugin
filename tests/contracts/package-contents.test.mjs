...
test("generated snapshots contain the exact clean product build plus the suite manifest and vendor lock", async (t) => {
  ...
  for (const productName of productNames) {
    await t.test(productName, async (t) => {
      ...
      assert.equal(pathsUnder(packageFiles, "skills/").filter((file) => file.endsWith("/SKILL.md")).length, productName === "game-design-studio" ? 26 : 25);
      assert.equal(pathsUnder(packageFiles, "skills/").filter((file) => file.endsWith("/SKILL.md") && !file.startsWith("skills/svg-infographic/")).length, productName === "game-design-studio" ? 25 : 24);
      ...
      for (const required of [
        ...
        "scripts/check-game-design-updates.mjs",
