...
test("clean Studio and Career builds import and execute the packaged quality runtime", async (t) => {
  for (const [productName, namespace, profileId, request] of [
    ["game-design-studio", "studio", "game-design-brief", { artifactId: "brief", goal: "master gdd", audience: ["production"], artifactType: "design-document", requestedFormat: "md", templateId: "game-design-brief" }],
    ["game-design-career", "career", "portfolio-case-study", { artifactId: "case-study", goal: "reverse design document", audience: ["recruiter"], artifactType: "career-document", requestedFormat: "md", templateId: "creative-design-portfolio" }],
  ]) {
    await t.test(productName, async (t) => {
      const stagingRoot = await mkdtemp(path.join(await realpath(tmpdir()), "quality-runtime-build-"));
      ...
      const application = await runtime.applyDocumentQualityProfile({ namespace, selectionIndex, pluginRoot: build.outputDir, request });
      assert.equal(application.selection.primaryProfileId, profileId);
      assert.equal(application.selection.selectionReason, "compatible-template-map-match");
