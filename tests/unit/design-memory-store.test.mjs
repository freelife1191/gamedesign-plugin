...
function mutationEvidenceMatches(error, { mutation, testId, sentinel }, operator) {
  return error instanceof assert.AssertionError
    ...
    && error.message.startsWith(`${sentinel}\n`)
    && process.env.DESIGN_MEMORY_MUTATION_EVIDENCE === "v1"
    && process.env.DESIGN_MEMORY_MUTATION_NAME === mutation
    && process.env.DESIGN_MEMORY_MUTATION_TEST_ID === testId
    && process.env.DESIGN_MEMORY_MUTATION_SENTINEL === sentinel;
}

function mutationEqual({ mutation, testId, sentinel }, actual, expected) {
  try { assert.equal(actual, expected, sentinel); }
  catch (error) {
    if (mutationEvidenceMatches(error, { mutation, testId, sentinel }, "strictEqual")) {
...
function mutationDeepEqual({ mutation, testId, sentinel }, actual, expected) {
  try { assert.deepEqual(actual, expected, sentinel); }
  catch (error) {
...
test("concurrent transitions and resolutions remain physical conflicts until one current-head resolution", async (t) => {
  ...
  assert.equal(scan.events.filter((item) => [verified.eventId, disputed.eventId].includes(item.eventId)).length, 2); const hasConcurrentAuthority = fold.memories.has(record.memory_id); mutationEqual({ mutation: "concurrent-fold-authority", testId: "concurrent-authority", sentinel: "MEM-MUT-CONCURRENT-AUTHORITY" }, hasConcurrentAuthority, false); assert.equal(fold.diagnostics.some((item) => item.code === "memory.concurrent_conflict"), true);
...
test("unsealed failpoint states stay non-authoritative and canonical retry prevents approved resurrection", { timeout: 20_000 }, async (t) => {
  ...
  const before = await scanMemoryEvents({ store }); assert.equal(before.complete, true); const hasUnsealedEvent = before.events.some((item) => item.eventId === eventId); mutationEqual({ mutation: "unsealed-claim-authority", testId: "unsealed-authority", sentinel: "MEM-MUT-UNSEALED-AUTHORITY" }, hasUnsealedEvent, false); assert.equal(foldMemoryEvents(before).memories.get(record.memory_id).record.status, "approved");
...
test("unsealed quarantine debris stays non-authoritative and canonical retry does not grow source files", { timeout: 20_000 }, async (t) => {
  ...
  assert.equal(created.status, "created"); assert.equal(created.eventId, markerId); assert.equal(created.relativePath, relativePath); assert.equal(present.status, "present"); const markerRetrySnapshot = await sourceTreeSnapshot(store); mutationDeepEqual({ mutation: "marker-retry-creates-debris", testId: "marker-retry-cardinality", sentinel: "MEM-MUT-MARKER-RETRY-CARDINALITY" }, markerRetrySnapshot, afterCreated);
...
test("the 10001st source-tree entry cannot hide an approval-invalidating event", { timeout: 30_000 }, async (t) => {
  ...
  const foldedMemoryCount = folded.memories.size; mutationEqual({ mutation: "scan-limit-partial-authority", testId: "scan-limit-authority", sentinel: "MEM-MUT-SCAN-LIMIT-AUTHORITY" }, foldedMemoryCount, 0); assert.equal(scan.complete, false); assert.deepEqual(scan.events, []); assert.deepEqual(scan.quarantines, []); assert.equal(scan.entriesScanned <= 10_000, true); assert.deepEqual(scan.diagnostics, [{ code: "memory.scan_limit_exceeded" }]);
});
...
test("sequential event and quarantine retry cardinality does not grow source files", async (t) => {
  ...
  const eventRetrySnapshot = await sourceTreeSnapshot(store); mutationDeepEqual({ mutation: "retry-creates-debris", testId: "retry-cardinality", sentinel: "MEM-MUT-RETRY-CARDINALITY" }, eventRetrySnapshot, afterEvent);
