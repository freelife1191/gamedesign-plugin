...
async function captured(t) {
  ...
  const event = { eventId: "playtest-1", type: "playtest-finding", summary: "반격 수단이 없다.", applicability: "보스전", exclusions: "퍼즐", artifactTypes: ["combat"], relatedIds: ["boss"], tags: ["counterplay"], actor: "author", sources: [{ artifact_id: "artifact", locator: "evidence.yml#x", sha256: (await import("node:crypto")).createHash("sha256").update(bytes).digest("hex") }] }; const receipt = issueDurableCaptureReceipt({ event, classificationResult: { classification: "durable-finding" } }); const capture = await captureDesignMemory({ workspaceRoot: root, config, projectId: "wind-island", lane: "studio", now: new Date("2026-08-01T00:00:00Z"), event, receipt });
  return { root, capture };
}
