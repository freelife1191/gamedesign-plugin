...
async function captured(t) {
  ...
  const capture = await captureDesignMemory({ workspaceRoot: root, config, projectId: "wind-island", lane: "studio", now: new Date("2026-08-01T00:00:00Z"), event: { eventId: "playtest-1", type: "playtest-finding", summary: "반격 수단이 없다.", applicability: "보스전", exclusions: "퍼즐", artifactTypes: ["combat"], relatedIds: ["boss"], tags: ["counterplay"], actor: "author", sources: [{ artifact_id: "artifact", locator: "evidence.yml#x", sha256: (await import("node:crypto")).createHash("sha256").update(bytes).digest("hex") }] } });
  return { root, capture };
}
...
test("verified candidate with human actor and reason is the only direct approval path", async (t) => {
  ...
  await maintainDesignMemory({ workspaceRoot: root, config, action: "verify", memoryId: capture.memoryId, actor: "reviewer", reason: "sources current", now: new Date("2026-08-02T00:00:00Z") });
  await assert.rejects(() => maintainDesignMemory({ workspaceRoot: root, config, action: "approve", memoryId: capture.memoryId, actor: "ai-agent", reason: "reviewed", now: new Date("2026-08-03T00:00:00Z") }), (error) => error?.code === "memory.maintenance");
  const approved = await maintainDesignMemory({ workspaceRoot: root, config, action: "approve", memoryId: capture.memoryId, actor: "reviewer", reason: "human review", now: new Date("2026-08-03T00:00:00Z") });
...
test("sweep appends expired and stale transitions without rewriting prior records", async (t) => {
  const { root, capture } = await captured(t);
  const result = await maintainDesignMemory({ workspaceRoot: root, config, action: "sweep", actor: "reviewer", reason: "scheduled review", now: new Date("2026-10-02T00:00:00Z") });
...
test("list and lint are read-only, quarantine permanently excludes the complete memory, and rebuild matches retrieval bytes", async (t) => {
  ...
  await maintainDesignMemory({ workspaceRoot: root, config, action: "quarantine", memoryId: capture.memoryId, actor: "reviewer", reason: "unsafe", now: new Date("2026-08-02T00:00:00Z") });
