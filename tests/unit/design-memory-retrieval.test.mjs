...
test("global and local malformed reservation matrix consumes slots without granting authority", async (t) => {
  ...
  for (const scope of ["global", "local"]) {
    for (const [name, mutateFile] of mutations) {
      ...
      const healthy = await publishMemoryLogGeneration({ store, sourceTreeSha256: "b".repeat(64), logBytes: Buffer.from("# healthy\n"), limits: { maxGenerationReservations: 2, maxIdentityInstances: 2 } }); assert.equal(healthy.complete, true, `${scope}/${name}: ${JSON.stringify(healthy)}`);
    }
