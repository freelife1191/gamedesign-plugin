...
- 플러그인 상대 경로 최장값은 179자다. Windows 실설치가 통과했으므로 파일명 축약은 하지 않는다.
...
4. 경로 길이 예산. 플러그인 상대 경로를 200자로 제한한다. 현재 최장값은 179자이므로 파일명 축약 없이 통과하며, 이후 증가를 막는 회귀 방지 장치로 동작한다.
...
- 패키지에 BOM, invalid UTF-8, CRLF, NFC·대소문자 충돌, 200자 초과 경로가 없다.
...
Windows 실설치가 현재 경로 길이로 통과했으므로 파일명을 줄이지 않는다. 경로 길이 예산만 두어 증가를 막는다.
