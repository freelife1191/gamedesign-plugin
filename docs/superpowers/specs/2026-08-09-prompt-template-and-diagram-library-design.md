...
기본 템플릿은 90개다. 각 템플릿은 App와 CLI 실행문을 모두 제공하므로 180개의 복사 가능한 실행문을 만든다.
...
기본 스킬 템플릿, 기존 사례, 레시피와 suite 사례를 합친 문서화 단위는 146개다. 146개 모두 App와 CLI 실행문을 제공하므로 사용자가 복사할 수 있는 실행문은 총 292개다. 중복 문구를 복제하지 않고 카탈로그 ID와 관련 문서 링크로 연결한다.
...
- App·CLI 복사 가능 실행문이 146개 사례 단위에 각각 존재해 총 292개다.
