...
- Studio 첫 브리프
- Career 첫 경력 계획
- 완성한 Studio 기획서를 취업용 포트폴리오 사례로 정리하는 연계 흐름

세 번째 경로는 `증거 후보`처럼 구현 내부에서만 의미가 분명한 용어를 사용자 제목이나 핵심 설명으로 쓰지 않는다. 사용자는 다음 네 가지를 바로 이해할 수 있어야 한다.

1. 준비물: Studio에서 완성하고 사람이 검토한 기획서 하나
2. 하는 일: 문제, 맡은 범위, 선택 이유와 검증 결과를 포트폴리오 사례로 재구성
3. 얻는 결과: 포트폴리오 본문, 개인 기여 요약, 면접 답변 소재와 공개 전 점검표
4. 검토 경계: 실제 참여 범위와 공개 권한은 작성자가 확인하며 플러그인이 자동 승인하지 않음

표시 제목은 `완성한 게임 기획을 취업용 포트폴리오 사례로 정리하기`를 사용한다. 요청문은 두 플러그인을 한 문장에 추상적으로 호출하지 않고, 먼저 Studio 결과의 공개 가능 정보를 검토한 뒤 Career 포트폴리오 스킬로 재구성하는 순서를 명시한다. `creative-design-portfolio`는 결과 폴더의 영문 ID로만 보조 표시한다.
...
- Studio 산출물은 공개 가능성 검토 뒤에만 Career 포트폴리오 사례의 입력으로 사용한다.
