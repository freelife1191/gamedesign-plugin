...
교차가 실제로 필요해진 시점에만 한 번 확인한다. 세션 시작 훅을 건드리지 않는다. 확인은 선택이 아니다. 인계가 필요하다고 적은 영수증은 이 조회를 실제로 돌린 뒤에만 공개한다. 상대가 없는데 "인계 필요"라고 적으면 사용자는 오지 않을 증거를 기다리고, 공급 단계를 조용히 자기 제품 일감으로 흡수하면 그 단계는 근거 없이 수행된다.
