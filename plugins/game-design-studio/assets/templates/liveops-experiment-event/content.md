---
title: 라이브옵스 실험 및 이벤트
artifact_id: liveops-experiment-event
quality_profile: liveops-event-experiment-plan
version: 1
---
# 라이브옵스 실험 및 이벤트 {#liveops-experiment-event}

## 실험 설계 기준 {#experiment-contract}

모든 experiment-id에는 반증 가능한 가설 하나, 대조군, 변경 변수 하나, 대상 표본, 제외 조건, 기간, 성공 지표, 가드레일 지표, 분석 담당자, 의사결정 규칙이 필요하다.

## 보호 및 롤백 게이트 {#protection-and-rollback-gate}

필요한 동의 또는 정책 근거, 플레이어 보호 지표, 중단 조건, 롤백 담당자, 복구 절차, 오염 여부 확인, 결론이 나지 않을 때의 계획이 없으면 시작하지 않는다.

## 작성 기록 {#working-record}

| 항목 ID | 상태 | 근거 또는 다음 작업 | 담당자 |
| --- | --- | --- | --- |
| `experiment-id` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `hypothesis` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `control` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `single-variable` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `sample` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `duration` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `success` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `guardrail` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `stop-condition` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `rollback` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
