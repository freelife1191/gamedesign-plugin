---
title: 접근성 및 플랫폼 매트릭스
artifact_id: accessibility-platform-matrix
quality_profile: accessibility-platform-matrix
version: 1
---
# 접근성 및 플랫폼 매트릭스 {#accessibility-platform-matrix}

## 플랫폼 접근성 매트릭스 {#platform-access-matrix}

플랫폼과 핵심 행동별로 입력 방식, 키 재지정, 포커스 이동, 시각·청각 대체 수단, 글자 크기, 모션·햅틱 옵션, 성능 예산, 안전 영역, 오프라인·중단 시 동작, 검증 결과를 기록한다.

## 접근성 완료 게이트 {#accessibility-completion-gate}

지원 플랫폼에서 필수 상태·신호·입력 경로·복구 경로·동등한 감각 대체 수단 중 하나라도 없거나 검증되지 않았다면 핵심 행동은 완료되지 않았다.
...
| `critical-action` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `input-method` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `focus-navigation` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `visual-alternative` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `audio-alternative` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `text-scale` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `performance` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `offline-interruption` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `verification` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
