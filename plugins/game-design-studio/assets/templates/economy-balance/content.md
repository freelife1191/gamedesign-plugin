...
version: 1
---
# 경제 및 밸런스 {#economy-balance}

## 경제 설계 기준 {#economy-contract}

각 resource-id에 획득처, 소비처, 목표 보유량, 성장 소요 시간, 교환 규칙, 세분화 한도, 인플레이션·악용 위험, 텔레메트리, 담당자, 검토 주기를 기록한다.

## 가격·확률·복구 게이트 {#price-probability-and-recovery-gate}

실제 가격 표시, 확률 고지, 천장 또는 보장 동작, 구매 확인, 환불 범위, 이상 탐지, 중단 조건, 롤백이 명확하고 현행 정책 근거가 연결되기 전에는 출시를 승인하지 않는다.

## 작성 기록 {#working-record}

| 항목 ID | 상태 | 근거 또는 다음 작업 | 담당자 |
| --- | --- | --- | --- |
| `resource-id` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `source` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `sink` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `target-inventory` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `progression-time` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `real-price` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `probability` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `pity` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `inflation-risk` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
