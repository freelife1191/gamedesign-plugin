...
version: 1
---
# 규칙 및 예외 매트릭스 {#rule-exception-matrix}

## 규칙·예외 우선순위 {#rule-and-exception-order}

모든 rule-id와 exception-id에 결정적인 우선순위, 조건, 권한, 동시성 동작, 충돌 해결 방식, 실패 상태, 복구 조치, 테스트 케이스를 부여한다.

## 충돌 처리 경계 {#conflict-boundary}

서로 모순되는 규칙을 문서 순서나 명시되지 않은 직관으로 해결하지 않는다. 해결되지 않은 우선순위는 구현 승인을 막고 결정 기록으로 남긴다.

## 작성 기록 {#working-record}

| 항목 ID | 상태 | 근거 또는 다음 작업 | 담당자 |
| --- | --- | --- | --- |
| `rule-id` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `priority` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `condition` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `exception-id` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `concurrency` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `authority` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `failure` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `recovery` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `test-case` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
