...
## 시스템 규칙 명세 {#system-contract}

각 rule-id에 입력, 사전 조건, 규칙, 상태 전이, 출력, 피드백, 예외, 우선순위, 동시성, 실패, 복구, 악용 사례, UI 상태, 담당자를 명시한다.

## 우선순위 및 런타임 매핑 {#precedence-and-runtime-mapping}

규칙 우선순위는 명시적으로 해결한다. 각 기획 항목을 데이터 출처, 테이블 또는 스키마 키, 런타임 소비처, 권한, 동기화 규칙, 검증 방법, 롤백 경로에 매핑한다.
...
| `precondition` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `state-transition` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `output` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `feedback` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `precedence` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `exception` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `failure-recovery` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
| `data-runtime-mapping` | not-observed | 승인 전에 프로젝트별 값과 근거 위치를 기록한다. | artifact-owner |
