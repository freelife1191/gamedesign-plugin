...
```claim
{
  ...
  "type": "contextual",
  "basis": "synthesis",
  "guidance": "플랫폼별 입력 방식과 UI 상태·정보 구조를 정리하고, 기기와 플랫폼의 화면·사용 영역에 맞춰 표시와 상호작용 차이를 설계한다.",
  "sourceIds": ["systems-7ccf322de528", "systems-081b21e5d10c", "systems-91d23bacb462"],
  "applicability": "PC·모바일·콘솔 등 입력 장치, 화면 크기, 사용 가능 영역이 다른 빌드의 UI 기획에 적용한다.",
  "counterexamples": ["입력 장치와 표시 영역이 같은 단일 플랫폼 빌드는 공통 UI 규칙만으로 충분할 수 있다."],
  "limitations": "로컬 원문의 입력·UI·기기 대응 원칙을 종합한 것이며 플랫폼별 요구사항을 증명하지 않는다. 각 플랫폼의 최신 공식 문서를 별도로 검토한다."
}
```
...
```claim
{
  ...
  "type": "evergreen",
  "basis": "synthesis",
  "guidance": "핵심 재미 가설은 프로토타입으로 검증하고, 마일스톤마다 목표 완성도와 검증 허들을 확인하며, 허들을 통과하지 못할 때의 중단 조건을 명시한다.",
  "sourceIds": ["career-7143bd076592"],
  "applicability": "프리프로덕션의 핵심 재미 검증과 프로덕션 마일스톤 중간 점검에 적용한다.",
  "counterexamples": ["프로덕션 후반에는 큰 틀 변경이 어려우므로 핵심 가설 검증을 그 시점까지 미루는 방식에는 적합하지 않다."]
}
```
