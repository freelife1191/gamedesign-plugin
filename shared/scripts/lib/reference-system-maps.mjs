...
export function validateReferenceSystemMaps({ maps, inventorySystemIds } = {}) {
  ...
  for (const map of maps) {
    ...
    if (!sorted(map.connections.map(({ connectionId } = {}) => connectionId), id) || !map.connections.every((connection) => exact(connection, ["connectionId", "fromNodeId", "toNodeId", "connectedSystemIds"]) && id(connection.connectionId) && nodes.has(connection.fromNodeId) && nodes.has(connection.toNodeId) && sorted(connection.connectedSystemIds, id) && connection.connectedSystemIds.every((systemId) => systems.has(systemId)))) return false;
    let previousLoopId;
