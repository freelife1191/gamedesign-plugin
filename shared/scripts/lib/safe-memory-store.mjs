...
async function safeReplace(parent, destination, bytes) {
  ...
  try {
    ...
    const temporaryPathStats = await targetFile(temporary); if (!temporaryPathStats || !same(temporaryPathStats, temporaryStats)) unsafe(); await assertIdentities(parent.identities); const current = await targetFile(destination); if ((original === undefined) !== (current === undefined) || original && !same(original, current)) unsafe(); await rename(temporary, destination); const published = await targetFile(destination); if (!published || !same(published, temporaryStats)) unsafe(); await syncDirectory(parent.path); await assertIdentities(parent.identities);
...
export async function readMemoryFile({ store, relativePath, maxBytes = MAX_BYTES } = {}) {
  ...
  try { const opened = await handle.stat(); if (!opened.isFile() || !same(opened, prior) || opened.size > maxBytes) unsafe(); const bytes = await handle.readFile(); const after = await handle.stat(); await assertIdentities(parent.identities); const current = await targetFile(parent.target); if (!same(after, opened) || !current || !same(current, prior) || bytes.byteLength > maxBytes) unsafe(); return bytes; } finally { await handle.close(); }
}
...
export async function writeMemoryFileAtomic({ store, relativePath, bytes, policy = "replace" } = {}) {
  ...
  try { await assertIdentities(parent.identities); handle = await open(temporary, constants.O_WRONLY | constants.O_CREAT | constants.O_EXCL | (constants.O_NOFOLLOW ?? 0), 0o600); await handle.writeFile(bytes); await handle.sync(); const temporaryStats = await handle.stat(); await handle.close(); handle = undefined; const tempPathStats = await targetFile(temporary); if (!tempPathStats || !same(tempPathStats, temporaryStats)) unsafe(); if (typeof policy?.beforeRename === "function") await policy.beforeRename({ destination: parent.target, temporary }); await assertIdentities(parent.identities); const current = await targetFile(parent.target); if ((original === undefined) !== (current === undefined) || original && !same(original, current)) unsafe(); await rename(temporary, parent.target); const published = await targetFile(parent.target); if (!published || !same(published, temporaryStats)) unsafe(); await syncDirectory(parent.path); await assertIdentities(parent.identities); } catch (error) { await handle?.close().catch(() => {}); try { await assertIdentities(parent.identities); await unlink(temporary); } catch {} throw error; }
}

export async function moveMemoryFileAtomic({ store, from, to } = {}) {
  const source = await parentFor(store, from); const sourceStats = await targetFile(source.target); if (!sourceStats) throw new Error("Memory file does not exist."); const destination = await parentFor(store, to, true); if (await targetFile(destination.target)) throw new Error("Memory destination already exists."); await assertIdentities([...source.identities, ...destination.identities]); const current = await targetFile(source.target); if (!current || !same(current, sourceStats)) unsafe(); await rename(source.target, destination.target); const moved = await targetFile(destination.target); if (!moved || !same(moved, sourceStats)) unsafe(); await assertIdentities([...source.identities, ...destination.identities]);
}
...
export async function ensureMemoryGitExclusion({ workspaceRoot, gitMode, runGit } = {}) {
  ...
  const existing = await readMemoryLikeFile(parent, exclude).catch((error) => error?.code === "ENOENT" ? Buffer.alloc(0) : Promise.reject(error)); const text = existing.toString("utf8"); if (text.includes(MARKER_START) || text.includes(MARKER_END)) return { status: "present" };
  const separator = existing.byteLength === 0 || text.endsWith("\n") ? "" : "\n"; await safeReplace(parent, exclude, Buffer.concat([existing, Buffer.from(`${separator}${MARKER}`)])); return { status: "added" };
}
...
async function readMemoryLikeFile(parent, target) {
  ...
  try { const opened = await handle.stat(); if (!opened.isFile() || !same(opened, prior) || opened.size > MAX_BYTES) unsafe(); const bytes = await handle.readFile(); const after = await handle.stat(); await assertIdentities(parent.identities); const current = await targetFile(target); if (!same(after, opened) || !current || !same(current, prior) || bytes.byteLength > MAX_BYTES) unsafe(); return bytes; } finally { await handle.close(); }
}
