...
function sameFileState(left, right) { return same(left, right) && left.size === right.size && left.mtimeMs === right.mtimeMs && left.ctimeMs === right.ctimeMs; }
...
async function readGitExcludeSnapshot(handle) {
  const before = await handle.stat(); if (!before.isFile() || before.size > MAX_BYTES) fail("Unsafe git exclude.");
...
async function verifyGitExcludePathname(expected, handle) { const pathname = await safeFile(expected); const opened = await handle.stat(); if (!pathname || !sameFileState(pathname, opened)) fail("Unsafe git exclude."); }

export async function ensureMemoryGitExclusion({ workspaceRoot, gitMode, runGit, beforeAppend, beforeFinalRecheck } = {}) {
  ...
  try { const root = await canonicalDirectory(workspaceRoot); const commonPath = path.isAbsolute(common) ? common : path.join(root.path, common); const commonRoot = await canonicalDirectory(commonPath); const infoPath = path.join(commonRoot.path, "info"); await inspectExistingPathChain(infoPath); await mkdir(infoPath, { recursive: false, mode: 0o700 }).catch((error) => error.code === "EEXIST" ? undefined : Promise.reject(error)); await canonicalDirectory(infoPath); const expected = path.join(infoPath, "exclude"); if (path.resolve(exclude) !== path.resolve(expected)) return { status: "warning", code: "memory.git_exclude_path" }; const lock = `${expected}.game-design-memory-exclude.lock`; let lockHandle; try { lockHandle = await open(lock, constants.O_WRONLY | constants.O_CREAT | constants.O_EXCL | (constants.O_NOFOLLOW ?? 0), 0o600); } catch { return { status: "warning", code: "memory.git_exclude_lock" }; } try { const existingStat = await safeFile(expected); let handle; if (existingStat) { handle = await open(expected, constants.O_RDWR | (constants.O_NOFOLLOW ?? 0)); const opened = await handle.stat(); if (!opened.isFile() || !same(opened, existingStat) || opened.size > MAX_BYTES) fail("Unsafe git exclude."); } else handle = await open(expected, constants.O_RDWR | constants.O_CREAT | constants.O_EXCL | (constants.O_NOFOLLOW ?? 0), 0o600); try { const initial = await readGitExcludeSnapshot(handle); await verifyGitExcludePathname(expected, handle); const text = initial.bytes.toString("utf8"); const begin = (text.match(/# game-design-plugin:memory:begin/gu) ?? []).length; const end = (text.match(/# game-design-plugin:memory:end/gu) ?? []).length; if (begin !== end || begin > 1) return { status: "warning", code: "memory.git_exclude_marker" }; if (begin === 0) { if (typeof beforeAppend === "function") await beforeAppend(); const beforeAppendSnapshot = await readGitExcludeSnapshot(handle); if (!sameGitExcludeSnapshot(initial, beforeAppendSnapshot)) fail("Unsafe git exclude."); const suffix = Buffer.from(`${text && !text.endsWith("\n") ? "\n" : ""}${MARKER}`); let offset = initial.bytes.byteLength; while (offset < initial.bytes.byteLength + suffix.byteLength) offset += (await handle.write(suffix, offset - initial.bytes.byteLength, suffix.byteLength - (offset - initial.bytes.byteLength), offset)).bytesWritten; await handle.sync(); if (typeof beforeFinalRecheck === "function") await beforeFinalRecheck(); const final = await readGitExcludeSnapshot(handle); if (!final.bytes.equals(Buffer.concat([initial.bytes, suffix]))) fail("Unsafe git exclude."); } await verifyGitExcludePathname(expected, handle); return { status: "ready" }; } finally { await handle.close(); } } finally { await lockHandle.close(); await rm(lock, { force: true }); } } catch { return { status: "warning", code: "memory.git_exclude" }; }
}
