...
export function memoryRecordRelativePath(record) {
  if (!record || !safeId(record.memory_id) || !safeId(record.project_id) || !["style-preference", "project-fact", "decision", "design-lesson", "career-lesson", "external-note"].includes(record.kind) || !["candidate", "verified", "approved", "expired", "rejected", "disputed", "superseded", "stale"].includes(record.status)) unsafe();
...
export async function writeMemoryFileAtomic({ store, relativePath, bytes, policy = "replace" } = {}) {
  ...
  try { await assertIdentities(parent.identities); handle = await open(temporary, constants.O_WRONLY | constants.O_CREAT | constants.O_EXCL | (constants.O_NOFOLLOW ?? 0), 0o600); await handle.writeFile(bytes); await handle.sync(); const temporaryStats = await handle.stat(); await handle.close(); handle = undefined; const tempPathStats = await targetFile(temporary); if (!tempPathStats || !same(tempPathStats, temporaryStats)) unsafe(); if (typeof policy?.beforeRename === "function") await policy.beforeRename({ destination: parent.target, temporary }); if (typeof policy?.beforePublish === "function") await policy.beforePublish({ destination: parent.target, temporary }); await assertIdentities(parent.identities); const current = await targetFile(parent.target); if ((original === undefined) !== (current === undefined) || original && !sameFileVersion(original, current) || original) unsafe(); await link(temporary, parent.target); const published = await targetFile(parent.target); if (!published || !same(published, temporaryStats)) unsafe(); await unlink(temporary); await syncDirectory(parent.path); await assertIdentities(parent.identities); } catch (error) { await handle?.close().catch(() => {}); try { await assertIdentities(parent.identities); await unlink(temporary); } catch {} throw error; }
}
