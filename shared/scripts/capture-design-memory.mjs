...
function eventHash(event) { const { receipt, ...payload } = event ?? {}; return createHash("sha256").update(JSON.stringify(payload), "utf8").digest("hex"); }
export function issueDurableCaptureReceipt({ event, classificationResult } = {}) { const classification = classificationResult?.classification; const explicit = event?.type === "explicit-preference"; const durable = classification === "durable-finding" && !explicit && Array.isArray(event?.sources) && event.sources.length > 0 && [event.applicability, event.exclusions].every((value) => safeText(value) && value.trim().length >= 2); const preference = classification === "explicit-user-preference" && explicit && safeText(classificationResult?.instructionContext) && safeText(event?.summary); if (!event || !Object.keys(classificationResult ?? {}).every((key) => ["classification", "instructionContext"].includes(key)) || !(durable || preference)) throw Object.assign(new Error("Human authority is required."), { code: "memory.human_authority_required" }); const receipt = Object.freeze({}); captureReceipts.set(receipt, { hash: eventHash(event), classification }); return receipt; }
...
export async function captureDesignMemory({ workspaceRoot, config, projectId, lane, event, receipt, now = new Date(), disabledForRequest = false } = {}) {
  ...
  const authority = receipt && captureReceipts.get(receipt); if (!authority || authority.hash !== eventHash(event) || authority.classification !== (explicit ? "explicit-user-preference" : "durable-finding") || ![event.summary, event.applicability, event.exclusions].every(safeText) || lists.some((item) => item === null) || (!explicit && !Array.isArray(event.sources))) return { status: "skipped" };
