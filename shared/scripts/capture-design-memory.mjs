...
export async function captureDesignMemory({ workspaceRoot, config, projectId, lane, event, now = new Date(), disabledForRequest = false } = {}) {
  ...
  if (!trustedUpstream(event, explicit) || ![event.summary, event.applicability, event.exclusions].every(safeText) || lists.some((item) => item === null) || (!explicit && !Array.isArray(event.sources))) return { status: "skipped" };
