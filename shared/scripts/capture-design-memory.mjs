...
function trustedUpstream(event, explicit) { return event?.actorType === "human" && event?.humanAttested === true && safeText(event.actor) && event.classification === (explicit ? "explicit-user-preference" : "durable-finding"); }
...
export function issueDurableCaptureReceipt({ event, classification } = {}) { if (!event || !["durable-finding", "explicit-user-preference"].includes(classification) || (classification === "explicit-user-preference") !== (event.type === "explicit-preference")) throw Object.assign(new Error("Human authority is required."), { code: "memory.human_authority_required" }); const receipt = Object.freeze({}); captureReceipts.set(receipt, { hash: eventHash(event), classification }); return receipt; }
