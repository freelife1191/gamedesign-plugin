...
function human(actor, actorType, humanAttested) { return typeof actor === "string" && actor.trim() && actorType === "human" && humanAttested === true; }
...
async function transition({ workspaceRoot, config, action, memoryId, actor, actorType, humanAttested, reason: why, now, chosenParentEventId, observedParentEventIds }) {
  if (!safeId(memoryId) || !human(actor, actorType, humanAttested) || !reason(why)) fail(); const store = await storeFor(workspaceRoot, config, true); if (!store) fail(); const state = await current(store, memoryId); const time = iso(now);
  if (action === "resolution") {
    if (!Array.isArray(observedParentEventIds) || observedParentEventIds.length < 2 || observedParentEventIds.some((id, index) => typeof id !== "string" || (index && observedParentEventIds[index - 1] >= id)) || !observedParentEventIds.includes(chosenParentEventId)) fail(); const observed = state.currentHeads.filter((item) => observedParentEventIds.includes(item.eventId)); if (observed.length !== observedParentEventIds.length || !observed.some((item) => item.eventId === chosenParentEventId)) fail(); const chosen = observed.find((item) => item.eventId === chosenParentEventId); const event = { schema_version: 1, event_type: "resolution", action: "resolution", memory_id: memoryId, parent_event_ids: observedParentEventIds, chosen_parent_event_id: chosenParentEventId, effective_at: time, actor, reason: why, record: chosen.record }; event.operation_id = memoryOperationId(event); const appended = await appendMemoryEvent({ store, eventDocument: canonicalMemoryEventDocument(event, chosen.sections) }); return withLog({ ...appended, memoryId, store }, workspaceRoot, config, now);
  }
  ...
  try { const appended = await appendMemoryEvent({ store, eventDocument: canonicalMemoryEventDocument(event, state.head.sections) }); return withLog({ ...appended, memoryId, store }, workspaceRoot, config, now); } catch (error) { if (error?.code) throw error; fail(); }
}
async function lintMemoryStore({ workspaceRoot, config, now }) { const store = await storeFor(workspaceRoot, config, false); if (!store) fail("memory.store_unavailable"); const scan = await scanMemoryEvents({ store }); if (!scan.complete) fail("memory.scan_incomplete"); const fold = foldMemoryEvents(scan, { now }); const diagnostics = [...scan.diagnostics, ...fold.diagnostics]; for (const memory of fold.memories.values()) { const checked = await validateMemorySourceBindings(memory.record, { workspaceRoot }); if (!checked.ok) diagnostics.push({ code: "memory.orphan_source" }); } return { status: "ready", memories: [...fold.memories.keys()].sort(), diagnostics }; }
async function listMemoryRecords({ workspaceRoot, config }) { const store = await storeFor(workspaceRoot, config, false); if (!store) fail("memory.store_unavailable"); const scan = await scanMemoryEvents({ store }); if (!scan.complete) fail("memory.scan_incomplete"); const fold = foldMemoryEvents(scan); const all = new Map(); for (const item of scan.events) (all.get(item.memoryId) ?? all.set(item.memoryId, []).get(item.memoryId)).push(item); return { status: "ready", memories: [...all.entries()].map(([memoryId, events]) => { const headsForMemory = heads(events); const memory = fold.memories.get(memoryId); return { memoryId, status: memory?.record.status ?? "concurrent-conflict", headEventIds: headsForMemory.map((item) => item.eventId), headStatuses: headsForMemory.map((item) => item.record.status) }; }).sort((a, b) => a.memoryId.localeCompare(b.memoryId)), diagnostics: [...scan.diagnostics, ...fold.diagnostics] }; }
async function sweep({ workspaceRoot, config, actor, actorType, humanAttested, reason: why, now }) { if (!human(actor, actorType, humanAttested) || !reason(why)) fail(); const store = await storeFor(workspaceRoot, config, true); if (!store) fail(); const scan = await scanMemoryEvents({ store }); if (!scan.complete) fail("memory.scan_incomplete"); const fold = foldMemoryEvents(scan, { now }); const date = iso(now).slice(0, 10); const changed = []; for (const [memoryId, value] of fold.memories) { const status = value.record.status === "candidate" && value.record.expires_at < date ? "expired" : value.record.kind === "external-note" && value.record.review_after < date ? "stale" : null; if (status) changed.push(await transition({ workspaceRoot, config, action: status, memoryId, actor, actorType, humanAttested, reason: why, now })); } return { status: "ready", changed }; }
async function quarantine({ workspaceRoot, config, memoryId, actor, actorType, humanAttested, reason: why, now }) { if (!safeId(memoryId) || !human(actor, actorType, humanAttested) || !reason(why)) fail(); const store = await storeFor(workspaceRoot, config, true); if (!store) fail(); const state = await current(store, memoryId); const target = state.events.sort((a, b) => a.eventId.localeCompare(b.eventId))[0]; if (!target) fail(); const appended = await appendQuarantineMarker({ store, targetMemoryId: memoryId, targetEventId: target.eventId, targetRelativePath: target.relativePath, observedSha256: createHash("sha256").update(target.bytes).digest("hex"), reasonCode: "memory.user-quarantine", actor, now }); return withLog({ ...appended, memoryId, store }, workspaceRoot, config, now); }
...
export async function maintainDesignMemory({ workspaceRoot, config, action, memoryId, actor, actorType, humanAttested, reason, chosenParentEventId, observedParentEventIds, now = new Date() } = {}) {
  ...
  if (action === "quarantine") return quarantine({ workspaceRoot, config, memoryId, actor, actorType, humanAttested, reason, now });
  if (action === "sweep") return sweep({ workspaceRoot, config, actor, actorType, humanAttested, reason, now });
  return transition({ workspaceRoot, config, action, memoryId, actor, actorType, humanAttested, reason, chosenParentEventId, observedParentEventIds, now });
}
