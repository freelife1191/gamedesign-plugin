...
const KOREAN_UI = Object.freeze([
  ...
  ["Workflow diagram • Archify로 제작", "워크플로 도식 • Archify로 제작"], ["Data flow diagram • Archify로 제작", "데이터 흐름 도식 • Archify로 제작"], ["Data-flow diagram • Archify로 제작", "데이터 흐름 도식 • Archify로 제작"], ["• Hover to trace •", "• 가리켜 경로 보기 •"], ["route • Click to focus •", "경로 • 클릭하여 초점 맞추기 •"], ["zoom •", "확대/축소 •"], ["radar •", "레이더 •"], ["views •", "안내 보기 •"], ["play story •", "이야기 재생 •"], ["theme •", "테마 •"], ["export", "내보내기"], ["Create yours ↗", "직접 만들기 ↗"],
  ["LENS", "렌즈"], ["MAP", "지도"], ["PATH", "경로"], ["READ 100%", "읽기 100%"], ["S cycles", "S 키로 전환"], ["Drag to pan", "끌어서 이동"], ["Click node", "노드 클릭"], ["6s motion", "6초 움직임"], ["vector", "벡터"], ["data store", "데이터 저장소"], ["async batch", "비동기 묶음"], ["primary data", "주 데이터"], ["policy / PII", "정책 / 개인정보"],
]);
...
function koreanRuntimeLocalizer() {
  ...
  return `<script data-archify-ko-localizer>(function(){document.documentElement.lang='ko';const d=new Map(${replacements});const p=[[/^(\\d+) nodes?$/,'$1개 노드'],[/^(\\d+) links?$/,'$1개 연결'],[/^(\\d+) stops?$/,'$1개 지점'],[/^(\\d+) nodes · full map$/,'$1개 노드 · 전체 지도'],[/^(\\d+) semantic nodes · (\\d+) relationships · (\\d+) guided views$/,'의미 노드 $1개 · 관계 $2개 · 안내 보기 $3개'],[/^Focus (.+) from Semantic Radar$/,'의미 레이더에서 $1에 초점'],[/^Focus (.+), (\\d+) related connections?$/,'$1에 초점 · 관련 연결 $2개'],[/^Focus (.+)$/,'$1에 초점'],[/^Inspect (.+), (\\d+) nodes?$/,'$1 살펴보기 · 노드 $2개'],[/^Inspect relationship (\\d+) of (\\d+): (.+) to (.+), (.+)\\. Press Enter for details\\.$/,'관계 $1/$2 살펴보기: $3 → $4, $5. 자세히 보려면 Enter를 누르세요.'],[/^(.+) → (.+) · connects to$/,'$1 → $2 · 연결'],[/^Open chapter (\\d+) of (\\d+): (.+)\\. Chapter focus delta: (\\d+) stay, (\\d+) enter, (\\d+) leave$/,'장 $1/$2 열기: $3. 초점 변화: 유지 $4, 진입 $5, 이탈 $6'],[/^(.+) — =(\\d+) \\+(\\d+) −(\\d+) chapter focus$/,'$1 — 장 초점: 유지 $2, 진입 $3, 이탈 $4'],[/^Chapter (\\d+) \\/ (\\d+)$/,'장 $1 / $2'],[/^Walk (\\d+) authored chapters and their real relationships\\.$/,'작성된 장 $1개와 실제 관계를 따라갑니다.'],[/^Visual style: (.+)\\. Choose visual style$/,'시각 스타일: $1. 시각 스타일 선택'],[/^(.+), (\\d+) nodes?$/,'$1 · 노드 $2개'],[/^(.+) Diagram$/,'$1 도식']];const kinds=new Map([['backend','백엔드'],['cloud','클라우드'],['database','데이터베이스'],['external','외부'],['messagebus','메시지 버스'],['security','보안']]);const q=[...d].filter(([a])=>a!==a.toLowerCase()).sort((a,b)=>b[0].length-a[0].length);const f=s=>{const m=/^(\\s*)([\\s\\S]*?)(\\s*)$/.exec(s);let v=d.get(m[2])??m[2];for(const [a,b] of p)v=v.replace(a,b);v=v.replace(/, connects to\\. 자세히/,' , 연결. 자세히');for(const [a,b] of q)v=v.split(a).join(b);v=v.replace(/^(backend|cloud|database|external|messagebus|security)( · )/,(_,k,sep)=>kinds.get(k)+sep);return m[1]+v+m[3]};const w=n=>{if(n.nodeType===3){const v=f(n.nodeValue);if(v!==n.nodeValue)n.nodeValue=v}else if(n.nodeType===1){for(const a of ['aria-label','title','aria-description','data-tooltip','placeholder'])if(n.hasAttribute(a)){const v=f(n.getAttribute(a));if(v!==n.getAttribute(a))n.setAttribute(a,v)}for(const c of n.childNodes)w(c)}};w(document.documentElement);new MutationObserver(ms=>ms.forEach(m=>{if(m.type==='attributes')w(m.target);else m.addedNodes.forEach(w)})).observe(document.documentElement,{childList:true,subtree:true,characterData:true,attributes:true,attributeFilter:['aria-label','title','aria-description','data-tooltip','placeholder']})})();</script>`;
}
