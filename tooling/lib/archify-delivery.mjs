...
const f=s=>{const m=/^(\s*)([\s\S]*?)(\s*)$/.exec(s);let v=d.get(m[2])??m[2];for(const [a,b] of p)v=v.replace(a,b);v=v.replace(/, connects to\. 자세히/,' , 연결. 자세히');v=v.replace(/^(backend|cloud|database|external|messagebus|security)( · )/,(_,k,sep)=>kinds.get(k)+sep);return m[1]+v+m[3]};
