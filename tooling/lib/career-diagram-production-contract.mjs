...
export function validateCareerDiagramProductionBatch(allSources, routing) {
  ...
  const selectableSkills = new Set(SKILL_ROWS.map((row) => row[3]).filter((skill) => skill !== "apply-document-quality-profile" && skill !== "orchestrate-game-design-career"));
  for (const source of careerSources.filter(({ scope }) => scope === "game-design-career-skill")) {
    ...
    for (const route of source.semantic.next_routes) {
      if (route.target === "<selected-skill>") {
        if (selectableSkills.size === 0) throw new TypeError(`${source.id} selected route has no installed target`);
      } else if (!installedSkills.has(route.target)) {
